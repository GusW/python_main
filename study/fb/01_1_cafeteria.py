...
def getMaxAdditionalDinersCount(N: int, K: int, M: int, S: List[int]) -> int:
    ...
    S.sort()
    fobbidenSeats = [list(range(j - K, j + K + 1)) for j in S]
    flat = set([i for sub in fobbidenSeats for i in sub])
    res = set()
    for i in set(range(1, N + 1)) - flat:
        y = set(range(i - K, i + K + 1))
        if not res.intersection(y):
            res.add(i)
            flat |= y
