...
def energy_at(pixels, x, y):
    ...
    previous_x = pixels[y][x0]
    next_x = pixels[y][x1]

    previous_y = pixels[y0][x]
    next_y = pixels[y1][x]

    dxr = math.pow(next_x.r - previous_x.r, 2)
    dxg = math.pow(next_x.g - previous_x.g, 2)
    dxb = math.pow(next_x.b - previous_x.b, 2)
    dx = dxr + dxg + dxb

    dyr = math.pow(next_y.r - previous_y.r, 2)
    dyg = math.pow(next_y.g - previous_y.g, 2)
    dyb = math.pow(next_y.b - previous_y.b, 2)
    dy = dyr + dyg + dyb
...
def compute_energy(pixels):
    ...
    for y in range(len(pixels)):
        ...
        for x in range(len(pixels[0])):
            x_deque.append(energy_at(pixels, x, y))

        y_deque.append(x_deque)
