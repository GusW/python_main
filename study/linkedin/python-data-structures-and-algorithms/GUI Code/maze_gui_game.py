...
try:
    # Not part of standard Library. Install with `pip install playsound`.
    import playsound
...
class MazeGUI:
    ...
    def init_screen(self):
        ...
        # Disable animation - we will update screen manually.
    ...
    def game_over(self):
        ...
        self.score_turtle.write(
            "GAME OVER", align="center", font=config.SCORE_FONT)
    ...
    def draw_maze(self, obstacles):
        for obstacle in obstacles:
            self.builder.goto(helpers.screen_coords_from_grid_pos(
                obstacle, self.maze_dimensions))
            self.builder.stamp()

    def draw_piece(self, piece, pos, stamp=False):
        piece.goto(helpers.screen_coords_from_grid_pos(
            pos, self.maze_dimensions))
        if stamp:
            piece.stamp()
    ...
    def start_round(self):
        self.update_score_display()
        # Unbind (s)tart key to prevent event stacking.
...
if __name__ == "__main__":
    maze_grid, maze_dimensions, maze_obstacles, player_start_pos, opponent_start_pos = helpers.read_maze_from_file(
        config.MAZE_FILE)
    MazeGUI(maze_grid, maze_dimensions, maze_obstacles,
            player_start_pos, opponent_start_pos)
    turtle.done()
