# myCompany coding assessment - Budget notifications
myCompany at its core is a platform where fashion retailers can advertise their products. They specify a budget, i.e. an upper limit to the amount of money they are prepared to spend each month in exchange for an equivalent amount of exposure on our platform. Once that budget is exhausted, shops go _offline_ and their products are no longer advertised on myCompany until a new month begins and the advertising budget is replenished.
...
* `a_online`: Specifies, whether a shop's products are currently being listed on the myCompany website. `1` means they are listed, `0` means they aren't.
...
* `a_budget_amount`: Signifies the monetary value a shop is willing to spend with myCompany in a given month.
